...
func InitServer(ctx context.Context) (*Server, error) {
	...
	m.Router.Post("/api/user/balance/withdraw", middlewares(h.CreateWithdrawHandler)) // запрос на списание баллов с накопительного счёта в счёт оплаты нового заказа;
	m.Router.Get("/api/user/withdrawals", middlewares(h.ReadWithdrawsHandler))        // получение информации о выводе средств с накопительного счёта пользователем.
